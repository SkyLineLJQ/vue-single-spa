...
let customProps = { customProps: storeModule };
...
singleSpa.registerApplication( //注册微前端服务
    ...
    location => location.pathname.startsWith('/vue-test'), // 配置微前端模块前缀
    customProps
);
...
singleSpa.registerApplication( //注册微前端服务
    ...
    async () => {
        ...
        // () => import('xxx/main.js')
        let myvuepro = null;
        await getManifest('http://127.0.0.1:9090/manifest.json', 'app').then(() => {
            myvuepro = window.myvuepro;
        });
        return myvuepro;
    },
    location => location.pathname.startsWith('/myvuepro'), // 配置微前端模块前缀
    customProps
);
